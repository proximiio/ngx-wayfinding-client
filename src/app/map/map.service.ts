import { Injectable } from '@angular/core';
import { Subject } from 'rxjs';
import { GeopointModel } from './map.model';
import { HttpClient } from '@angular/common/http';

const GEO_API_ROOT = 'https://api.proximi.fi/v4/geo';

@Injectable({providedIn: 'root'})
export class MapService {
  mapboxPositronStyleUrl = 'https://maps.tilehosting.com/styles/positron/style.json?key=IsnIsFy7POjHakrA56Qd';
  bluedot = `data:image/png;base64,`;
  mapCenterListener = new Subject<{location: GeopointModel, zoom: number}>();
  floorplansListener = new Subject<any[]>();

  constructor(
    private http: HttpClient) {}

  getRoute(start: any, end: any, accessibility: boolean, skipRender: boolean, token: string) {
    const lngStart = start.coordinates[0];
    const latStart = start.coordinates[1];
    const levelStart = start.level;
    const lngFinish = end.coordinates[0];
    const latFinish = end.coordinates[1];
    const levelFinish = end.level;
    const url = `${GEO_API_ROOT}/route/${lngStart},${latStart},${levelStart}%3B${lngFinish},${latFinish},${levelFinish}?token=${token}&accessibility=${accessibility}`;

    return this.http
      .get(url);
  }

  centerizeMap(location: GeopointModel, zoom?: number) {
    this.mapCenterListener.next({
      location: location,
      zoom: zoom
    });
  }

  setFloorplans(floors: any) {
    this.floorplansListener.next(floors);
  }
}
