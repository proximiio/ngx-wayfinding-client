var Settings = {
  token: "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..ho5_3mO6VMOVpAMVWO2O_3WBSzGz_wAUFe0sC8ULeZU",
  basepath: "/supermarket", // in case of change it's required to change that also at ./src/environments and ./angular.json files
  port: "6032", // in case of change it's required to change that also at ./src/environments
  proximi_api: 'https://api.proximi.fi'
};

module.exports = Settings;
